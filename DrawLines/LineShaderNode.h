...
public:
   ...
   bool GetLocations() 
   {
      ...
      {
         printf("LineShaderNode: Error getting ortho matrix location\n");
         return false;
      }
      m_positionLoc = glGetAttribLocation(m_shaderProgram.GetProgram(), "position");
      if (m_positionLoc < 0)
      {
         printf("LineShaderNode: Error getting vertex position location\n");
         return false;
      }
      m_vertexColorLoc = glGetAttribLocation(m_shaderProgram.GetProgram(), "vertexColor");
      if (m_vertexColorLoc < 0)
      {
         printf("LineShaderNode: Error getting vertex color location\n");
         return false;
      }
      return true;
   }
    ...
    * @param  sceneState   Current scene state.
	...
	virtual void Draw(SceneState& sceneState)
	{
      ...
      sceneState.m_vertexColorLoc = m_vertexColorLoc;
